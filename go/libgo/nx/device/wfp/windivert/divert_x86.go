...
var _x86_WinDivert32_sys []byte

//go:embed x86/WinDivert.dll
var _x86_WinDivert_dll []byte

// getEmbedDriverFileData ...
func getEmbedDriverFileData() (sys, dll []byte) {
	return _x86_WinDivert32_sys, _x86_WinDivert_dll
}
...
func open(filter string, layer Layer, priority int16, flags uint64) (h *Handle, err error) {
	...
	if windows.Handle(hd) == windows.InvalidHandle {
		// logx.E("[divert] open %v", err.(windows.Errno))
