...
struct BPF {
    ...
    bool Matches(gopacket::CaptureInfo ci, bytez<> data);
};
...
struct Handle : public io::xx::closer_t {
    ...
    R<bytez<>, gopacket::CaptureInfo, error> ReadPacketData();
    ...
    R<bytez<>, gopacket::CaptureInfo, error> ZeroCopyReadPacketData();

    // WritePacketData calls pcap_sendpacket, injecting the given data into the pcap handle.
    error WritePacketData(bytez<> data);
