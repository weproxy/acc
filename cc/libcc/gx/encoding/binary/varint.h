...
bytez<> AppendUvarint(bytez<> buf, uint64 x) {
    while (x >= 0x80) {
...
int PutUvarint(bytez<> buf, uint64 x) {
    int i = 0;
    while (x >= 0x80) {
        buf[i] = byte(x) | 0x80;
        x >>= 7;
...
R<uint64, int> Uvarint(bytez<> buf) {
    uint64 x = 0;
    uint s = 0;
    // for i, b := range buf {
...
bytez<> AppendVarint(bytez<> buf, int64 x) {
...
int PutVarint(bytez<> buf, int64 x) {
    uint64 ux = uint64(x) << 1;
    if (x < 0) {
        ux = ~ux;
    }
