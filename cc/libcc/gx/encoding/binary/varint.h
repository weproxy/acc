...
slice<byte> AppendUvarint(slice<byte> buf, uint64 x) {
    while (x >= 0x80) {
...
int PutUvarint(slice<byte> buf, uint64 x) {
    int i = 0;
    while (x >= 0x80) {
        buf[i] = byte(x) | 0x80;
        x >>= 7;
...
R<uint64, int> Uvarint(slice<byte> buf) {
    uint64 x = 0;
    uint s = 0;
    // for i, b := range buf {
...
slice<byte> AppendVarint(slice<byte> buf, int64 x) {
...
int PutVarint(slice<byte> buf, int64 x) {
    uint64 ux = uint64(x) << 1;
    if (x < 0) {
        ux = ~ux;
    }
...
//	n  < 0: value larger than 64 bits (overflow)
//	        and -n is the number of bytes read
R<int64, int> Varint(slice<byte> buf) {
