...
namespace xx {
// The xx::readOp:: constants describe the last action performed on
...
extern error errNegativeRead;
extern error errUnreadByte;

const int maxInt = int(~uint(0) >> 1);
...
struct Buffer {
    ...
    xx::readOp lastRead;  // last read operation, so that Unread* can work correctly.
   ...
   public:
    ...
    void Truncate(int n) {
        ...
        b.lastRead = xx::readOp::opInvalid;
        if (n < 0 || n > b.Len()) {
   ...
   private:
    ...
    int grow(int n) {
        ...
        if (!b.buf && n <= xx::smallBufferSize) {
            b.buf = make(n, xx::smallBufferSize);
            return 0;
        }
        int c = cap(b.buf);
        if (n <= c / 2 - m) {
   ...
   public:
    ...
    R<rune, int, error> ReadRune() {
        ...
        if (c < utf8::RuneSelf) {
            b.off++;
            b.lastRead = xx::readOp::opReadRune1;
            return {rune(c), 1, nil};
        }
        AUTO_R(r, n, utf8::DecodeRune(b.buf(b.off)));
        b.off += n;
        b.lastRead = xx::readOp(n);
        return {r, n, nil};
    }
    ...
    error UnreadRune() {
        auto& b = *this;
        if (b.lastRead <= xx::readOp::opInvalid) {
...
Ref<Buffer> NewBuffer(slice<byte> buf) {
...
// In most cases, new(Buffer) (or just declaring a Buffer variable) is
// sufficient to initialize a Buffer.
Ref<Buffer> NewBufferString(const string& s) {
    auto p = NewRef<Buffer>();
    // p->buf = buf;
    return p;
}
